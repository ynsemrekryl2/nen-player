#!/usr/bin/env bash
# NEN-011 ölçümü: NEN-009'un (async progress + cooperative cancellation)
# Kotlin/JVM paritesi.
#
#   bash scripts/spike-async-jvm.sh                 # ölçüm + testler, release
#   bash scripts/spike-async-jvm.sh --debug
#   bash scripts/spike-async-jvm.sh --test-only      # yalnız invariant testleri
#   bash scripts/spike-async-jvm.sh --measure-only   # yalnız baseline ölçümü
#
...
#
# scripts/spike-async.sh (Swift) ile aynı 4 adım, farkla: adım 2 --language
# kotlin üretir, adım 3 swift test yerine ./gradlew test.

set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CORE="$ROOT/core"
PKG="$ROOT/core/spikes/spike-async-cancel/jvm-harness"
OUT="$PKG/generated"
...
echo "▶ 2/4  spike-async-uniffi-bindgen generate --language kotlin"
rm -rf "$OUT"
mkdir -p "$OUT/lib"
...
cp "$DYLIB" "$OUT/lib/libspike_async_cancel.dylib"

cat > "$OUT/README.md" <<'GEN'
# ÜRETİLEN DİZİN — elle düzenlemeyin, commit etmeyin

`bash scripts/spike-async-jvm.sh` üretir. `.gitignore` → `/core/spikes/**/generated/`.
GEN
...
echo "▶ 4/4  ölçüm — checkpoint aralığı × cancellation latency"
echo
