...
| **M6** | Real Providers | OpenSubtitles resmi API + OpenAI + OpenRouter; varsayılan OpenRouter/Luna, yalnız OpenAI upstream'i; secure credential storage; katalogda gerçek adaylar. | 🔵 sıradaki |
...
| **S3** | Çeviri kalite hedefi: "anlaşılır" mı, "yayın kalitesi" mi? | ✅ cevaplandı (2026-09-11) | M5'in yapısal doğruluğu korunur. **Yayın kalitesi yalnız EN→TR için** `NEN-126`'daki telif-temiz 24 cue korpusunda ölçülür: 24/24 cue zorunlu düzeltmesiz, anlam tersine dönmesi/atlama/ekleme/isim-terim tutarsızlığı sıfır. Varsayılan OpenRouter `openai/gpt-5.6-luna`, yalnız OpenAI upstream'i; Luna geçmezse ayrı task Terra yükseltmesiyle aynı kabul tekrarlanır. Karar: [ADR-0019](adr/0019-real-translation-provider-boundary.md) |
...
| **S9** | Aynı medya için birden fazla AI çeviri saklanabilir mi? | ✅ cevaplandı (2026-09-11) | **Diskte ve menüde evet.** Provider/model kimliğiyle ayrışan doğrulanmış artifact'ler hedef dil grubunda `AI · sağlayıcı · model profili` etiketiyle ayrı gösterilir; en yenisi başlangıçta seçilir, kullanıcı diğerini seçebilir. Projeksiyon `NEN-118`'dedir. Karar: [ADR-0019](adr/0019-real-translation-provider-boundary.md) |
